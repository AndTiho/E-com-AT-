from tests.conftest import product_sample_1


def test_category(category_sample_1, category_sample_2):
    assert category_sample_1.name == "Телевизоры"
    assert category_sample_1.description == "Современный телевизор, который позволяет наслаждаться просмотром, станет вашим другом и помощником"
    ...
    assert category_sample_2.description == "Смартфоны, как средство не только коммуникации, но и получения дополнительных функций для удобства жизни"
